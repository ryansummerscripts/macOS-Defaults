...
display_configuration() {
    ...
    echo "  Model Name .... ${CY}${SP_MODEL_NAME}${NC}"
    echo "  Processor ..... ${CY}${SP_PROCESSOR_NAME}${NC}"
    if [[ "$DEVICE_TYPE" == "Desktop" ]] || [[ "$DEVICE_TYPE" == "Laptop" ]]; then
        echo "  Virtualization: ${CY}None (Physical ${DEVICE_TYPE})${NC}"
    else
        echo "  Virtualization: ${CY}Yes (${SP_MODEL_NAME})${NC}"
    fi
    echo "  Username ...... ${CY}${user_name}${NC}"
    echo "  UID ........... ${CY}${uid}${NC}"
    echo "  Admin Status .. ${CY}${ADMIN_STATUS}${NC}"
    echo "${BO}${GR}════════════════════════════════════════════════════════════════════════════════${NC}"
    echo
}
                ...
                while true; do
                    ...
                    while true; do
                        ...
                        if [[ "$MACOS_MAJOR" -ge 26 ]]; then
                            ...
                            # This is already being used under the Archive Utility category
                            # if [[ "$MACOS_MINOR" -ge 4 ]]; then
                            #     # Tahoe 26.4+ — new dict-based keys                        
                            #     echo "🗜️  Archive Utility: Move archives to trash after expanding ${BL}(Tahoe 26.4+)${NC}"
                            #     defaults write com.apple.archiveutility "dearchive-move-after-location" -dict "Selection" "MoveToTrash"
                                
                            #     echo "🗜️  Archive Utility: Move files to trash after archiving ${BL}(Tahoe 26.4+)${NC}"
                            ...
                            #     echo "🗜️  Archive Utility: Move archives to trash after expanding ${BL}(Tahoe 26.3 & below)${NC}"
                            #     defaults write com.apple.archiveutility dearchive-move-after "~/.Trash"

                            #     echo "🗜️  Archive Utility: Move files to trash after archiving ${BL}(Tahoe 26.3 & below)${NC}"
                        ...
                        if [[ "$MACOS_MAJOR" -ge 15 ]]; then
                            ...
                            if [[ "$DEVICE_TYPE" != "VM" && "$arch_name" == "Apple Silicon" ]]; then
                        ...
                        if [[ "$MACOS_MAJOR" -ge 14 ]]; then
                            echo "----------------------------------------------------------------------------------------------------------------------------------"
                            echo "🆕 [${BO}NEW for${NC} ${GY}${GR}macOS Sonoma 14${NC}]"
                            ...
                            echo "🖥️  Desktop: Disable 'click wallpaper to show Desktop' ${GY}${GR}(Sonoma 14+)${NC}"
                            defaults write com.apple.WindowManager EnableStandardClickToShowDesktop -bool false

                            echo "🖥️  Desktop: Disable widgets on Desktop ${GY}${GR}(Sonoma 14+)${NC}"
                            ...
                            echo "🪄 Animations: Reduce Motion/Animations ${GY}${GR}(Sonoma 14+)${NC}"  # Reduces certain animations (i.e. the 'bubbly' spotlight search in Tahoe - but also affects mission control)
                        ...
                        if [[ "$MACOS_MAJOR" -le 11 ]] || [[ "$MACOS_MAJOR" -eq 10 && "$MACOS_MINOR" -lt 15 ]]; then
                            # Mojave (10.14) & below
                        ...
                        if [[ "$MACOS_MAJOR" -lt 11 ]] || [[ "$MACOS_MAJOR" -eq 10 && "$MACOS_MINOR" -lt 11 ]]; then
                        ...
                        # if [[ "$MACOS_MAJOR" -ge 14 ]]; then
                        #     echo "🚫 Reduce Motion/Animations ${GY}${GR}(Sonoma 14+)${NC}"     # Reduces certain animations (i.e. the 'bubbly' spotlight search in Tahoe - but also affects mission control)
                        #     default write com.apple.Accessibility ReduceMotionEnabled -bool true
                        #     defaults write com.apple.universalaccess reduceMotion -bool true

                        #     echo "🚫 Disable 'Auto-play animated images/GIFs ${GY}${GR}(Sonoma 14+)${NC}"
                        ...
                        # fi
                        # if [[ "$MACOS_MAJOR" -ge 14 ]]; then
                        #     echo "🚮 Delete verification codes after use ${GY}${GR}(Sonoma 14+)${NC}"
                        ...
                        if [[ "$MACOS_MAJOR" -lt 26 ]] || [[ "$MACOS_MAJOR" -ge 26 && "$MACOS_MINOR" -ge 4 ]]; then    # Tahoe 26.4+
